...
app.get('/exists', zValidator('query', uploadDto), async (c) => {
  ...
  const [file] = await db
    .select()
    .from(uploadsTable)
    .where(and(eq(uploadsTable.userId, jwt.id), eq(uploadsTable.sha, sha)))
    .execute()
...
app.post('/uploads', bodyLimit({ maxSize: 8 * 1024 * 1024, onError }), async (c) => {
  const db = initDbConnect(c.env)
  const jwt = c.get('jwt')

  const [user] = await db.select().from(usersTable).where(eq(usersTable.id, jwt.id)).execute()
  ...
  const [uploadsToday] = await db
    ...
    .from(uploadsTable)
    .where(and(eq(uploadsTable.userId, user.id), gte(uploadsTable.createdAt, tsToday)))
    .execute()
  ...
  if (result.status === 'rejected') {
    if (upload.status !== 'rejected') {
      c.executionCtx.waitUntil(db.delete(uploadsTable).where(eq(uploadsTable.id, upload.status)))
    }
